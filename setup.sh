...
function osnotify_install
{
    ...
    sudo "$1/env/bin/osnotify-install-service" "osnotify-proxy" "$2"
    sudo service osnotify-proxy start
}
...
function osnotify_ci
{
    ...
    osnotify_install "$HOME/osnotify" "$(id -un)"
}
...
if [ "$1" == "install" ]
then
    osnotify_install "$(readlink -f $2)" "$3"
fi
